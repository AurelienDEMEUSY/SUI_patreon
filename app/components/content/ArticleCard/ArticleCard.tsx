...
export function ArticleCard({ post, description }: ArticleCardProps) {
    const { metadata, images, isLoading: contentLoading, isUnlocked } = usePostContent(
        post.serviceObjectId,
        post.onChainPost
    );
    const imageUrl = isUnlocked && images.length > 0 ? images[0].url : DEFAULT_IMAGE;
    ...
    const shortDesc = description != null && description.length > 0
        ...
        : metadata?.text
            ? truncate(metadata.text, DESCRIPTION_MAX_LENGTH)
